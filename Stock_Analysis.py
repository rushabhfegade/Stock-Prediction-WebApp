            ...
            with st.spinner(f"Retrieving {self.ticker} Stock Analysis"):
                ...
                if trend:
                    ...
                    if trend_options[1] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Average Directional Movement Index (ADX) Indicator'):
                    ...
                    if trend_options[2] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Aroon Indicator'):
                    ...
                    if trend_options[3] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Commodity Channel Index (CCI) Indicator'):
                    ...
                    if trend_options[4] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Detrended Price Oscillator (DPO) Indicator'):
                    ...
                    if trend_options[5] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Exponential Moving Average (EMA) Indicator'):
                    ...
                    if trend_options[6] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Ichimoku Kinko Hyo (Ichimoku) Indicator'):
                    ...
                    if trend_options[7] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Know Sure Thing (KST) Indicator'):
                    ...
                    if trend_options[8] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Moving Average Convergence Divergence (MACD)'):
                    ...
                    if trend_options[9] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Mass Index (MI)'):
                    ...
                    if trend_options[10] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Parabolic Stop and Reverse (PSAR) Indicator'):
                    ...
                    if trend_options[11] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Simple Moving Average (SMA) Indicator'):
                    ...
                    if trend_options[12] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Schaff Trend Cycle (STC) Indicator'):
                    ...
                    if trend_options[13] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Trix Indicator'):
                    ...
                    if trend_options[14] in trend or trend_options[0] in trend:
                        with st.beta_expander(f'{self.ticker} Stock Vortex Indicator (VI)'):
                ...
                if momentum:
                    ...
                    if momentum_options[1] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f'{self.ticker} Stock Awesome Oscillator (AO) Indicator'):
                    ...
                    if momentum_options[2] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Kaufman's Adaptive Moving Average (KAMA) Indicator"):
                    ...
                    if momentum_options[3] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Percentage Price Oscillator (PPO)"):
                    ...
                    if momentum_options[4] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Rate of Change (ROC) Indicator"):
                    ...
                    if momentum_options[5] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Relative Strength Index (RSI) Indicator"):
                    ...
                    if momentum_options[6] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Stochastic Relative Strength Index (Stoch RSI) Indicator"):
                    ...
                    if momentum_options[7] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Stochastic Oscillator"):
                    ...
                    if momentum_options[8] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock True Strength Index (TSI) Indicator"):
                    ...
                    if momentum_options[9] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Ultimate Oscillator"):
                    ...
                    if momentum_options[10] in momentum or momentum_options[0] in momentum:
                        with st.beta_expander(f"{self.ticker} Stock Williams %R Indicator"):
                ...
                if volatility:
                    ...
                    if volatility_options[1] in volatility or volatility_options[0] in volatility:
                        with st.beta_expander(f"{self.ticker} Stock Average True Range (ATR)"):
                    ...
                    if volatility_options[2] in volatility or volatility_options[0] in volatility:
                        with st.beta_expander(f"{self.ticker} Stock Bollinger Bands"):
                    ...
                    if volatility_options[3] in volatility or volatility_options[0] in volatility:
                        with st.beta_expander(f"{self.ticker} Stock Donchian Channel"):
                    ...
                    if volatility_options[4] in volatility or volatility_options[0] in volatility:
                        with st.beta_expander(f"{self.ticker} Stock Keltner Channel"):
                    ...
                    if volatility_options[5] in volatility or volatility_options[0] in volatility:
                        with st.beta_expander(f"{self.ticker} Stock Ulcer Index"):
                ...
                if volume:
                    ...
                    if volume_options[1] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Accumulation/Distribution Index (ADI) Indicator"):
                    ...
                    if volume_options[2] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Chaikin Money Flow (CMF) Indicator"):
                    ...
                    if volume_options[3] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Ease of Movement (EoM) Indicator"):
                    ...
                    if volume_options[4] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Force Index (FI) Indicator"):
                    ...
                    if volume_options[5] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Money Flow Index (MFI) Indicator"):
                    ...
                    if volume_options[6] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Negative Volume Index (NVI) Indicator"):
                    ...
                    if volume_options[7] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock On-balance Volume (OBV) Indicator"):
                    ...
                    if volume_options[8] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Volume-price Trend (VPT) Indicator"):
                    ...
                    if volume_options[9] in volume or volume_options[0] in volume:
                        with st.beta_expander(f"{self.ticker} Stock Volume Weighted Average Price (VWAP)"):
                ...
                if return_selected:
                    ...
                    if return_options[1] in return_selected or return_options[0] in return_selected:
                        with st.beta_expander(f"{self.ticker} Stock Cumulative Return (CR)"):
                    ...
                    if return_options[2] in return_selected or return_options[0] in return_selected:
                        with st.beta_expander(f"{self.ticker} Stock Daily Log Return (DLR)"):
                    ...
                    if return_options[3] in return_selected or return_options[0] in return_selected:
                        with st.beta_expander(f"{self.ticker} Stock Daily Return (DR)"):
